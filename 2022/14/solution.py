...
class Map:
    ...
    @classmethod
    def from_paths(cls, paths: Sequence[Path]) -> "Map":
        max_row = max((position.row for path in paths for position in path))
        columns = [position.column for path in paths for position in path]
        min_column = min(columns)
        max_column = max(columns)

        map = [
            {column: "." for column in range(min_column, max_column + 1)}
...
class Simulation:
    ...
    def simulate_sand(self, start: Position):
        ...
        while next_position := self._get_next_position(current_position):
            ...
            if not self.map.within_map(current_position):
                break
    ...
    def run_simulation(self, start: Position) -> int:
        ...
        while sand := self.simulate_sand(start):
            if not self.map.within_map(sand):
                break
...
simulation = Simulation(Map.from_paths(paths))
